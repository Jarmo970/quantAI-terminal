...
st.sidebar.markdown("---")
st.sidebar.info(
    "💡 **Live Terminal:** Ticking clock runs locally every second. Market data auto-syncs every 60 seconds when open.")
...
# Cached data loader using yf.Ticker and explicit DatetimeIndex casting to resolve linter warnings
@st.cache_data(ttl=60)
def load_stock_data(ticker_symbol, start, end):
    try:
        ticker_obj = yf.Ticker(ticker_symbol)
        data_frame = ticker_obj.history(start=start, end=end, auto_adjust=True)

        if data_frame is not None and not data_frame.empty:
            dt_index = pd.DatetimeIndex(data_frame.index)
            if dt_index.tz is not None:
                data_frame.index = dt_index.tz_localize(None)
            return data_frame
    except Exception as e:
        st.error(f"API Error for {ticker_symbol}: {e}")

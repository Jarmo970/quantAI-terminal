...
# Cached data loader with custom requests session to bypass cloud rate-limits (NaN fix)
...
def load_stock_data(ticker_symbol, start, end):
    ...
    data_frame = yf.download(ticker_symbol, start=start, end=end, session=session, progress=False)
    if data_frame is not None and isinstance(data_frame.columns, pd.MultiIndex):
